...
struct GouraudShader : public IShader {
  ...
  virtual bool fragment(Vec3f bar_coordinates, TGAColor &color) {
    float intensity = vertex_lightIntensity *
                      bar_coordinates; // interpolate the intensity for current
                                       // pixel within the traingle.
...
struct DiffuseShader : public IShader {
  ...
  virtual bool fragment(Vec3f bar_coordinates, TGAColor &color) {
    float intensity =
        vertex_lightIntensity *
        bar_coordinates; // interpolate the intensity for current pixel
...
struct NormalMappingShader : public IShader {
  ...
  // light direciton transformation can be extract out of FS. We don't need to
  // compute it in FS everytime.
...
// If we want use TangentSpaceNormalMap, make sure we are using tangent space
// normal texture ! load_texture(filename, "_nm_tangent.tga", normalmap_) in
// model.cpp
struct TangentSpaceNormalMap : public IShader {
  ...
  mat<3, 3, float> varying_nrm{}; // normal per vertex in clip coordinates to be
                                  // interpolated by fragment shader
  ...
  virtual Vec4f vertex(int iface, int nvert) {
    ...
    varying_nrm.set_col(
        nvert, proj<3>(uniform_MIT *
                       embed<4>(model->normal(iface, nvert),
                                0.f))); // normal is a vector, embed 0.f at end
    Vec4f GL_Vertex =
        uniform_M *
        embed<4>(model->vert(
            iface, nvert)); // vertex is a point, embed 1.0f by default.
