...
struct PhongShader : IShader {
    ...
    PhongShader(const vec3 light, const Model& m) : model(m) {
        //convert light directionto view space
        // because this is direction so 4th component is 0
    ...
    virtual std::pair<bool, TGAColor> fragment(const vec3 bar) const {
        // construct Darboux frame (TBN
        mat<2,4> E = { tri[1] - tri[0], tri[2] - tri[0]};
        mat<2,2> U = { varying_uv[1] - varying_uv[0], varying_uv[2] - varying_uv[0]};
        mat<2,4> T = U.invert() * E;
        mat<4,4> D = { normalized(T[0]), // tagent vector
                       normalized(T[1]), // bitangent vector
                       normalized(varying_nrm[0] * bar[0] + varying_nrm[1] * bar[1] + varying_nrm[2] * bar[2]), // interpolate norm
                       {0,0,0,1}};  // Darboux frame

        vec2 uv = varying_uv[0] * bar[0] + varying_uv[1] * bar[1] + varying_uv[2] * bar[2];
        vec4 n = normalized(D.transpose() * model.normal(uv));
        vec4 r = normalized(n * (n*l) * 2 - l); // normal in view coordinates (TBN * normal in tagent space)
        double ambient = .4;                    // ambient light intensity
        double diffuse = std::max(0., n * l);   // diffuse light intensity
        // view direction in view space is (0,0,1), so r*v = (0,0,1)*(r.x, r.y, r.z) = r.z
        // specular intensity, note that the camera lies on the z-axis (in eye coordinates), therefore simple r.z
        double specular = (1. + 3. * sample2D(model.specular(), uv)[0] / 255.) * std::pow(std::max(r.z, 0.), 35); 
        TGAColor gl_FragColor = sample2D(model.diffuse(), uv); // albedo
        for (int channel : {0, 1, 2})
            gl_FragColor[channel] = std::min<int>(255, gl_FragColor[channel] * (ambient + diffuse + specular)); // albedo affected ambient and diffuse, not very accurate
    ...
    for (int x=0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            ...
            bool lit = (fragment.z < -100 ||                                                            // it's the background or
                       ...
                       (p.z > zbuffer[static_cast<int>(p.x) + static_cast<int>(p.y) * shadoww] - .03)); // it is visible
            mask[x + y * width] = lit;
        }   
    }
